...
class Supergoat extends MY_Controller {
	...
	public function index($username='',$pixel='')
	{
		...
		if (count($exFunnel) > 2) {
			$funnel = $exFunnel[1];
			$username = $exFunnel[2];
			$dataMember = $this->member_model->get(array('username' => $exFunnel[2]));
			if ($dataMember['foto']) {
				
				$foto = 'http://member.saudagarkaya.com//assets/images/profile/'.$dataMember['foto'];
			}else{
				$foto = 'https://member.akademibisnis.id/assets/ui-member/images/logo/users.gif';
			}
			// $pixcel = $exFunnel[3];
		}else{
			$funnel = '';
